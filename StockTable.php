...
<head>
	...
	<title>
		White Lake Cheese Portal - Stock Table
	</title>
<?php
session_start();
ob_start();
...
session_destroy();
	...
	<?php
		...
		<tr>
			...
			{
				echo"<td style='text-align:center; border-top:2px solid blue'>";
			...
			echo"</td>
			<td style='text-align:center; border-top:2px solid blue'>";?>
				<?php echo "<button type='submit' name='Delete' title='Delete Product From System' value='DeleteProduct$i'><img src='delete_Icon.png' alt='Delete Button'></button>
			</td>";
			}
			$i++;
				}
		...
		if(isset($_POST['Edit']))
				{
						...
						$_SESSION['StockEditStored'] = $Stock_IDStore[$position];
						$UserStock_IDStoreEdit = $Stock_IDStore[$position];
						$editCheck = 1;
			...
			if($Account_Type == 5)
			{
				if(isset($_POST['Delete']))
				{
						...
						$position2 = substr($value2, 13, $lengthI2); //value of button is always Delete+variable $i by removing the word delete you are left with i which has been used to track which file is which 
						$_SESSION['position2'] = $position2; //store the position for use in getting correct file on next page
						//echo $position2;
						$_SESSION['StockDelete'] = $Stock_IDStore[$position2]; //store file array and position in session
						$UserStock_IDDelete = $Stock_IDStore[$position2];
						...
						if ($deleteCheck == 1)
						{
							...
							function myFunction() {
								var txt;
								if (confirm("Please Confirm Deletion of Product Details!")) {
							...
							<?php
							$DeleteDetail = "SELECT * FROM market_stock WHERE Stock_ID='$Stock_IDStore[$position2]'";
							$DeleteDetailResult = mysqli_query($conn,$DeleteDetail);
							while ($detaildeleterow = mysqli_fetch_array($DeleteDetailResult))
							{
								$StockDelID = $detaildeleterow[1];
								$deletesql = "Delete FROM market_stock WHERE Stock_ID='$StockDelID'";
								mysqli_query($conn,$deletesql);
							}
							$sql = "Delete FROM stock WHERE Stock_ID='$UserStock_IDDelete'";
							mysqli_query($conn,$sql);
							
							if ($conn->query($sql) === True)
							{
								//display complete message and link to homepage
								$_SESSION['StockDelete'] = "";
